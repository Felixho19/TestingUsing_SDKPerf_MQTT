...
if [ -z "$MQTT_URL" ]
then
	echo "Please set MQTT_URL to the URL for AWS IOT eg ssl://data.iot.eu-west-1.amazonaws.com:8883"
	exit -1
fi
...
if [ $EXPECT_TIME -gt 60 ]
then
        MSG_COUNT_FOR_LATENCY=$(( $TOTAL_RATE * 60 ))
fi
MESSAGE_COUNT=$(( $TOTAL_RATE * $WARM_UP + $MSG_COUNT_FOR_LATENCY ))

echo "Attempting to send to AWS at a rate of $MESSAGE_RATE, count=$MESSAGE_COUNT, size=$MESSAGE_SIZE, clients=$NUM_CLIENTS, QOS=$MQTT_SEND_QOS, WARM_UP=$WARM_UP"
...
if [ -f "aws_${TEST_NAME}.txt" ]
then
	echo "Skipping test as aws_${TEST_NAME}.txt exists"
else
$SDK_BIN -cip $MQTT_URL -as client-certificate -sslp TLSv1.2 \
	-sslts $KEYS_DIR/truststore.jks -ssltsp $PASSPHRASE  -ssltsf=jks \
	-sslks $KEYS_DIR/clientstore.jks -sslksp $PASSPHRASE -sslksf=jks \
	-sslpkp $PASSPHRASE -sslpka $ALIAS \
	...
	-lat -lg=5 -lb=4096 -lwu=$WARM_UP  \
	-d -cc $NUM_CLIENTS > aws_${TEST_NAME}.txt 2>&1
fi